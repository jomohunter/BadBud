...
{
    #[Route('/projet/{id}', name: 'app_nft_projet', methods: ['GET'])]
    public function nftsForProjet(NFTRepository $nFTRepository, Request $request): Response
    {
        // Récupérez l'ID du projet à partir des paramètres de la requête
        $projectId = $request->attributes->get('id');
        
        // Récupérez les NFT associés à ce projet à partir du référentiel
        $nfts = $nFTRepository->findBy(['projet' => $projectId]);

        // Afficher la liste des NFT pour ce projet dans un template Twig dédié
        return $this->render('nft/nfts_for_projet.html.twig', [
            'nfts' => $nfts,
        ]);
    }
